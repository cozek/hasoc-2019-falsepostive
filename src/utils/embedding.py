...
def make_embedding_matrix(word_to_idx, embd_vectors,
                          words, subword=False, pad_zero=True):
    """
    ...
    Args:
        ...
        subword (bool) : True for Models that can get embeddings for unknown words
    ...
    if subword:
        embedding_size = embd_vectors['a'].shape[0]
        final_embeddings = np.zeros((len(words), embedding_size))
        word_to_idx = {}
        for i, word in tqdm(enumerate(words), total=len(words)):
            final_embeddings[i, :] = embd_vectors[word]
            word_to_idx[word] = i
    else:
...
def make_embedding_matrix_old(word_to_idx, embd_vectors,
                          words, pad_zero=True):
    """
    Create embedding matrix for a specific set of words.
